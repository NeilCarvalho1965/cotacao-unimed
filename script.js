...
function parseCSV(data) {
    ...
    return linhas.map(l => {
        ...
        if (valores.length < cabecalho.length) return null;  // Ignora linhas incompletas
...
function gerarImagem(texto) {
    ...
    img.onload = function () {
        ...
        const logoWidth = 150;
        const logoHeight = 50;
        ctx.drawImage(img, (canvas.width - logoWidth) / 2, 20, logoWidth, logoHeight);

        ctx.fillStyle = '#000';
        ctx.font = '16px Arial';
        const lines = texto.split('\n');
        lines.forEach((line, idx) => {
            ctx.fillText(line, 20, 100 + idx * 20);
        });
        ...
        ctx.fillText('Sandra Regina – (41) 99981-7997', 20, canvas.height - 25);

        // Ícone WhatsApp
        const wppIcon = new Image();
        wppIcon.src = 'https://cdn-icons-png.flaticon.com/512/733/733585.png';
        wppIcon.onload = function() {
            ctx.drawImage(wppIcon, canvas.width - 50, canvas.height - 55, 40, 40);
